...
TInt THighScoreTable::Render(TInt aDifficulty, TInt aCount, TInt aX, TInt aY, BFont *aFont, TInt aColor, TInt aShadowColor) {
  ...
  for (TInt i   = 0; i < aCount; i++) {
    ...
    if (checkPlayerHighScore && *t[i].score <= *lastScore[gOptions->difficulty]) {
      bm->DrawStringShadow(gViewPort, buf, aFont, x, y, COLOR_MENU_TITLE, (TUint8)aShadowColor, COLOR_TEXT_TRANSPARENT, -4);
      checkPlayerHighScore = EFalse;
    } else {
      bm->DrawStringShadow(gViewPort, buf, aFont, x, y, (TUint8)aColor, (TUint8)aShadowColor, COLOR_TEXT_TRANSPARENT, -4);
    }
